...
fprintf('\n=== 7. Leistungsspektren (Subplots) und Wasserfalldiagramme ===\n');
...
% Leistungsspektren aller Fenster als Subplots in EINER gemeinsamen Figure
% (statt einer eigenen Figure je Fenster) -> direkter visueller Vergleich.
nW   = numel(results);
cols = ceil(sqrt(nW));
rows = ceil(nW / cols);
figure('Color','w','Position',[80 80 380*cols 300*rows]);
for iw = 1:nW
    R = results(iw);
    subplot(rows, cols, iw);
    plot_spectrum(R.f, R.psd, R.name, R.hrv);
end

% Wasserfalldiagramme bleiben je Fenster eine eigene Figure (3D-Plots
% lassen sich nicht sinnvoll als 2D-Subplot kombinieren).
for iw = 1:nW
    R = results(iw);
...
%   Zeichnet in die aktuelle Achse (der Aufrufer waehlt Figure/Subplot,
%   z.B. per subplot(...) vor dem Aufruf) -> so lassen sich die Spektren
%   mehrerer Fenster als Subplots in einer gemeinsamen Figure anordnen.
...
title(sprintf('Fenster: %s', window_name), 'FontSize', 10);
grid on; box on;
